...
create table if not exists `presentation` (`presId` int not null auto_increment, `studentId` int not null, constraint `FK_studentId` foreign key (`studentId`) references `user_APP`.`user`(`userId`), `teacherId` int not null , `tutorId` int not null , `finalDate` date, primary key (`presId`), constraint `FK_teacherId` foreign key (`teacherId`) references `user_APP`.`user`(`userId`), constraint `FK_tutorId` foreign key (`tutorId`) references `user_APP`.`user`(`userId`))ENGINE=INNODB;

create table if not exists `presentationDates` (`presId` int not null, `mode` varchar(30) not null  , `date` date not null, `teacherVote` int, `tutorVote` int, primary key(`presId`,`date`), constraint `FK_presId` foreign key (`presId`) references `presentation`(`presId`))ENGINE=INNODB;
